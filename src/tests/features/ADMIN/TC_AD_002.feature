Feature: TC_AD_002
  ...
  #   Then Verify whether the Registration tab is present and Manage User tab is not in the application as per admin case two
  ...
  # Scenario: Responder is unable to Upload the documents from Responder Side.
  #   Given Responder navigate to the Application and logged in as a discom user as per admin case two
  #   Then Verify whether the LOA Generation and Registration tab is not in the application as per admin case two

  # Scenario: Admin user giving LOA and Registration  Rights for Responder and manage user rights for Initiator
  #   Given Admin User navigates to the application and logs in as an admin as per Admin case two
  #   And Admin user giving LOA and Registration Rights for Responder and manage user rights for Initiator as per Admin case two

  Scenario: Responder Uploading the documents from Responder Side.
    Given Responder navigate to the Application and logged in as a discom user as per admin case two
    Then Verify whether the Responder have Uploaded the document successfull as per admin case two
