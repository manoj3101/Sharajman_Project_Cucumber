Feature: TC_AD_001
  ...
  #   Given User navigate to the Application and logged in as a discom user as Responder as per admin case one
  #   And User started Uploading the documents from Responder Side as per admin case one
  ...
  #   Given New User navigate to the Application and logged in as a discom user as initiator as per admin case one
  #   Then New Format D should be successfully Generated from initiator side as per admin case one
  ...
  # Scenario: New DISCOM user adds a new user for themselves
  ...
  #   And  Assigned new user verifies the password and logs in with their new credentials as per Admin Case One
