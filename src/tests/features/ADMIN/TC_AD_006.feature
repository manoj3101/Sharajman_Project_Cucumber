Feature: TC_AD_002
  ...
  Scenario: New Discom Member Create CFP and Publishing the CFP.
    Given Admin navigate to the application and login and fetching the transaction fee formula as per admin case six
  ...
  Scenario: New Discom Member Responding to the  CFP as a Responder.
    ...
    And New User started placing Response to the CFP as per admin case six
    Then New User Response CFP should be Placed successfully as per admin case six

  Scenario: Discom Member Awarding CFP and Generating LOA.
    Given User navigate to the Application and logged in as a discom user as initiator as per admin case six
    And User started generating the award and generating the LOA from initiator side as per admin case six
    Then User Awarding and Generate LOA should be successfull as per admin case six

  Scenario: New Discom Member Uploading the documents from Responder Side.
    Given New User navigate to the Application and logged in as a discom user as Responder as per admin case six
    Then New User Responder Uploading the documents should be successfull as per admin case six

  Scenario: Discom Member Genarating Format D as initiator
    Given User navigate to the Application and logged in as a discom user as initiator as per admin case six
    Then User Format D should be successfully Generated from initiator side as per admin case six

  Scenario: New Discom Member Genarating Format D as Responder
    Given New User navigate to the Application and logged in as a discom user as Responder as per admin case six
    Then New User Format D should be successfully Generated from Responder side as per admin case six
