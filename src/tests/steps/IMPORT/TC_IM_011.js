...
//@                                                     Scenario 1
//-------------------------------------------------------------------------------------------------------------------------
Given('Admin navigate to the application and login and fetching the transaction fee formula as per import case eleven', async function () {
    console.log("------------------------------------------------------------------------------------------------------");
    ...
    await transactionFee.fetch_Transaction_Fee(data.feeName, data.TC_11.Quantum_value); // Fetch the transaction fee formula

    await transactionFee.fetch_Success_Fee(data.successfee, data.TC_11.Quantum_value); // Fetch the Success fee formula
...
Given('User started creating Call for Proposal CFP as an initiator as per import case eleven', { timeout: 90 * 1000 }, async function () {
    ...
    await dashboardCFP.selectResponder(data.TC_11.multiple_responder, data.responder);
...
Then('CFP should be Placed successfully as Expected from responder as per import case eleven', { timeout: 120 * 1000 }, async function () {
    ...
    await dashboardCFP.energycalculation_initiator(DashboardCFP.imp_start_date, DashboardCFP.imp_end_date, data.TC_11.imp_start_time, data.TC_11.imp_end_time, data.TC_11.minQuantumValue1);
