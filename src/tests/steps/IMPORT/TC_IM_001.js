...
Given('Admin navigate to the application and login and fetching the transaction fee formula as per import case one', async function () {
    console.log("------------------------------------------------------------------------------------------------------");
    ...
    await transactionFee.fetch_Transaction_Fee(data.feeName, data.TC_01.Quantum_value); // Fetch the transaction fee formula

    await transactionFee.fetch_Success_Fee(data.successfee, data.TC_01.Quantum_value); // Fetch the Success fee formula

    await login.logout(); //Logout
});

Given('User navigate to the Application and logged in as a discom user as initiator as per import case one', async function () {
    console.log("-----------------------------------------INITIATOR-----------------------------------------");
    await login.login(data.user1, data.user1_password);

});

Given('User started creating Call for Proposal CFP as an initiator as per import case one', async function () {
    await home.clickCallForPropsal();

    await dashboardCFP.clickCreateCFP(data.TC_01.chooseCFP);

    await dashboardCFP.powerSwapping(data.TC_01.Power_swapping);

    await dashboardCFP.firstChoice(data.TC_01.Fisrt_choice);

    await dashboardCFP.resultPublish(data.TC_01.Result_Published);

    await dashboardCFP.importPeriod(data.TC_01.Quantum_value, data.TC_01.imp_start_date, data.TC_01.imp_end_date, data.TC_01.imp_start_time, data.TC_01.imp_end_time);

    await dashboardCFP.minimumQuantum(data.TC_01.Minimum_Quantum, data.TC_01.Minimum_QuantumValue);

    await dashboardCFP.exportPeriod(data.TC_01.exp_start_date, data.TC_01.exp_end_date, data.TC_01.exp_start_time, data.TC_01.exp_end_time);

    await dashboardCFP.publishing_time();

    await dashboardCFP.response_validityTime(data.TC_01.response_days, data.TC_01.response_hours, data.TC_01.response_mins);

    await dashboardCFP.awarding_time(data.TC_01.award_days, data.TC_01.award_hours, data.TC_01.award_mins);

    await dashboardCFP.loa_Issuance_time(data.TC_01.loa_issuance_days, data.TC_01.loa_issuance_hours, data.TC_01.loa_issuance_mins);

    await dashboardCFP.loa_Acceptance_time(data.TC_01.loa_acceptance_days, data.TC_01.loa_acceptance_hours, data.TC_01.loa_acceptance_mins);

    await dashboardCFP.otherDetails(data.TC_01.index, data.TC_01.Settlement_Price);

    await dashboardCFP.commentBox(data.TC_01.query_Box);

    await dashboardCFP.ceilingBaseReturn(data.TC_01.Ceiling_Base_Return, data.TC_01.Ceiling_Base_Return_value);

    await dashboardCFP.selectResponder(data.TC_01.multiple_responder, data.responder);

    await dashboardCFP.custom_Guests(data.TC_01.custom_Guests);

    await dashboardCFP.remarks(data.TC_01.remarks);

});

Then('Call for Proposal CFP should be Published successfully as per import case one', async function () {
...
Given('User started placing Response to the CFP as per import case one', { timeout: 120 * 1000 }, async function () {
...
Given('User started generating the award and generating the LOA from initiator side as per import case one', { timeout: 1200000 }, async function () {
...
Given('User started Uploading the documents from Responder Side as per import case one', async function () {
