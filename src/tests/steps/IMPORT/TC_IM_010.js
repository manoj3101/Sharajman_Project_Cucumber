...
//@                                                     Scenario 1
//-------------------------------------------------------------------------------------------------------------------------
Given('Admin navigate to the application and login and fetching the transaction fee formula as per import case ten', async function () {
    console.log("------------------------------------------------------------------------------------------------------");
    ...
    await transactionFee.fetch_Transaction_Fee(data.feeName, data.TC_10.Quantum_value); // Fetch the transaction fee formula

    await transactionFee.fetch_Success_Fee(data.successfee, data.TC_10.Quantum_value); // Fetch the Success fee formula
...
Then('CFP should be Placed as Expected from responder as per import case ten', async function () {
    ...
    await dashboardCFP.energycalculation_initiator(DashboardCFP.imp_start_date, DashboardCFP.imp_end_date, data.TC_10.imp_start_time, data.TC_10.imp_end_time, data.TC_10.minQuantumValue1);
